...
def set_angle(ID, angle, serial):
    ...
    print(f'Setting servo[{ID}] to {angle:.2f}')

    # serial = ServoSerial(port=port)
    # serial.open()

    servo = AX12() # Packet(servoType[args['type']])

    # pkt = servo.makeServoMovePacket(ID, angle)
    # val = angle2int(angle, degrees=True) # + le(speed)
    # print(val)
    # pkt = servo.makeWritePacket(ID, servo.GOAL_POSITION, val)
